...
const tunisNow = (instant = new Date()) => {
...
export async function markLateReservationsAsNoShow(prisma: PrismaClient, placeId?: number, instant = new Date()) {
  const now = tunisNow(instant);
  ...
  for (const row of rows) {
    ...
    const result = await prisma.reservation.updateMany({where: {id: row.id, status: ReservationStatus.CONFIRMED, checkedInAt: null}, data: {status: ReservationStatus.NO_SHOW, noShowMarkedAt: instant}});
